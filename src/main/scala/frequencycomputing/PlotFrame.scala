...
import MusicBox.Operation
import MusicBox.Store
import MusicBox.Write
import MusicBox.falseFrequency
import MusicBox.trueFrequency
...
object PlotFrame {
  ...
  def main(args: Array[String]) {
    ...
    val frequencyDist = falseFrequency - trueFrequency
    val storageFeq = trueFrequency + frequencyDist/3
    val storageFeq2 = trueFrequency + 2 * frequencyDist/3
    
    val intructions = Seq[Operation](
//        Write(true),
//         Store(storageFeq , 5.5),
         Write(true),
         Store(storageFeq2, 5.5))
        
    val machine = new MusicBox(intructions.iterator)
  ...
  private def createData(machine: MusicBox, time: Double, minRange: Double, maxRange: Double): XYDataset = {
    ...
    val data = xValues.map {
      x => 
        val y = machine.waveFunction(x, time)
        new XYDataItem(x, y)
    }
